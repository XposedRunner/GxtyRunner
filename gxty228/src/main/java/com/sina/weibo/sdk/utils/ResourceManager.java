...
public class ResourceManager {
    ...
    private static String getAppropriatePathOfDrawable(Context context, String str) {
        ...
        while (i < PRE_INSTALL_DRAWBLE_PATHS.length) {
            ...
            String stringBuilder = new StringBuilder(String.valueOf(PRE_INSTALL_DRAWBLE_PATHS[i])).append(HttpUtils.PATHS_SEPARATOR).append(str).toString();
        ...
        if (i3 >= 0) {
            return new StringBuilder(String.valueOf(PRE_INSTALL_DRAWBLE_PATHS[i3])).append(HttpUtils.PATHS_SEPARATOR).append(str).toString();
        }
        LogUtil.e(TAG, "Not find the appropriate path for drawable");
        return null;
    }
    ...
    private static String getCurrentDpiFolder(Context context) {
        ...
        if (i > 120 && i <= GlMapUtil.DEVICE_DISPLAY_DPI_NORMAL) {
            return DRAWABLE_MDPI;
        }
        if (i > GlMapUtil.DEVICE_DISPLAY_DPI_NORMAL && i <= GlMapUtil.DEVICE_DISPLAY_DPI_MEDIAN) {
            return DRAWABLE_HDPI;
        }
        if (i <= GlMapUtil.DEVICE_DISPLAY_DPI_MEDIAN || i > GlMapUtil.DEVICE_DISPLAY_DPI_HIGH) {
